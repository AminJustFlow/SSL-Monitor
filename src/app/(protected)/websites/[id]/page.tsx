...
export default async function Details({params}:{params:Promise<{id:string}>}){
  ...
  return <>
    ...
    {proxied&&<p className="error">This website appears to be behind {w.connectionType==="CLOUDFLARE"||w.usesCloudflare?"Cloudflare":"a CDN"}. The public certificate may have a different expiration date from the certificate installed on the origin server.</p>}
    {proxied&&!w.checkOriginCertificate&&<p className="panel">Public certificate checked successfully. Origin certificate has not been checked because no origin IP is configured.</p>}
    {mismatch&&<p className="error">The public and origin certificates have different expiration dates. This is normal for proxied websites, but the origin certificate must remain valid for strict TLS mode. The {origin!.expiresAt!<edge!.expiresAt!?"origin":"public / edge"} certificate expires first.</p>}
