...
export async function POST(request:Request){
  ...
  const body=await request.json() as {csrf?:string;originConnectHost?:string;originSniHostname?:string;hostname?:string;originPort?:number;connectionType?:string;usesCloudflare?:boolean};
  ...
  const result=await checkCertificate({connectHost,servername,port,allowPrivate:config().ALLOW_PRIVATE_ORIGIN_HOSTS,trustCloudflareOrigin:body.connectionType==="CLOUDFLARE"||body.usesCloudflare});
