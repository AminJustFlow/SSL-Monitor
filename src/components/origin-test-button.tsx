...
export function OriginTestButton({csrf}:{csrf:string}){
  ...
  async function test(event:React.MouseEvent<HTMLButtonElement>){
    ...
    try{const response=await fetch("/api/websites/test-origin",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({csrf,hostname,originConnectHost:data.get("originConnectHost"),originSniHostname:data.get("originSniHostname"),originPort:Number(data.get("originPort")||443)})});const result=await response.json() as {error?:string;status?:string;issuer?:Record<string,string>;expiresAt?:string;resolvedIp?:string;hostnameMatches?:boolean;authorized?:boolean};if(!response.ok)throw new Error(result.error||"Test failed");setMessage(`Status: ${result.status}; expires: ${result.expiresAt??"unknown"}; IP: ${result.resolvedIp??"unknown"}; hostname: ${result.hostnameMatches?"valid":"invalid"}; chain: ${result.authorized?"valid":"invalid"}.`);}catch(error){setMessage(error instanceof Error?error.message:"Test failed");}finally{setBusy(false);}
  }
