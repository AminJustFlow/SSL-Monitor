...
export const websiteSchema = z.object({
  ...
  tlsTimeoutMs: z.coerce.number().int().min(1000).max(60000),
  httpTimeoutMs: z.coerce.number().int().min(1000).max(60000),
  alertRecipients: csv.pipe(z.array(z.string().email())),
  alertThresholds: csv.transform(v => v.map(Number)).pipe(z.array(z.number().int().min(0).max(365))),
