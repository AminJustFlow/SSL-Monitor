...
export const websiteSchema = z.object({
  ...
  checkHttp: bool,
  usesCloudflare: bool,
  connectionType: z.enum(["DIRECT","CLOUDFLARE","CDN","UNKNOWN"]).default("UNKNOWN"),
  ...
  originPort: z.coerce.number().int().positive().default(443),
  edgeWarningDays: z.coerce.number().int().min(1).max(365).default(30),
  edgeCriticalDays: z.coerce.number().int().min(0).max(90).default(7),
  originWarningDays: z.coerce.number().int().min(1).max(365).default(30),
  originCriticalDays: z.coerce.number().int().min(0).max(90).default(7),
